...
from models import Notification, StudyGroup, StudyGroupMember, User
...
def join_study_group(
    ...
    notify_user_ids = {row.user_id for row in existing_member_ids}
    ...
    for member_id in notify_user_ids:
        db.add(
            Notification(
                user_id=member_id,
                message=f"{user.display_name} joined {group.course_code}"
            )
        )
